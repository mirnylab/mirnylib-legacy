...
class Genome(object):
    ...
    def __init__(self, genomePath, gapFile = 'gap.txt', chrmFileTemplate = 'chr%s.fa',
                 readChrms = ['#', 'X', 'Y', 'M']):
        '''__init__ : Initializes the genome, and calculates Genome properties (or loads them from cache). 
    ...
    def setResolution(self, resolution):
        if (resolution == -1) and hasattr(self,"resolution"): 
            for i in ["chrmLensBin","chrmStartsBinCont","chrmEndsBinCont","numBins",
                      "chrmIdxBinCont","posBinCont","cntrMidsBinCont","GCBin",
                      "unmappedBasesBin","binSizesBp","mappedBasesBin","resolution"]:                                
                exec("del self.%s" % i)
            return 
